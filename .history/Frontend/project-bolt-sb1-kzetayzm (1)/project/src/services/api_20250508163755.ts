...
const handleResponseError = async (
  ...
  if (!response.ok) {
    ...
    try {
      ...
      errorMessage = errorData.error || errorData.msg || defaultErrorMessage;
  ...
  chat: {
    ...
    createSession: async (
      ...
      try {
        ...
        // Call the backend API to create a session with timeout
        ...
        // Handle potential errors with our helper
        await handleResponseError(response, 'Failed to create chat session');
        ...
        // Create fallback session data for testing if needed
        const fallbackData = {
          session_id: sessionId,
        ...
        // Uncomment this to return fallback data instead of throwing
        // console.log('Using fallback session data:', fallbackData);
        // return fallbackData;
    ...
    getSessions: async (token: string): Promise<ChatSession[]> => {
      try {
        ...
        // Call the backend API to get sessions with timeout
        const response = await fetchWithTimeout(
          '/api/chat/sessions',
          {
            method: 'GET',
        ...
        // Uncomment to return empty sessions instead of throwing
        // console.log('Returning empty sessions array as fallback');
        // return [];
    ...
    sendMessage: async (
      ...
      try {
        ...
        // Make an actual API call to our backend with timeout
        const response = await fetchWithTimeout(
          '/api/chat',
          {
            method: 'POST',
        ...
        // Handle potential errors with our helper
        await handleResponseError(response, 'Failed to send message');
    ...
    getHistory: async (token: string): Promise<ChatMessage[]> => {
      try {
        ...
        // Make a real API call to the backend with timeout
        const response = await fetchWithTimeout(
          '/api/chat/history',
          {
            method: 'GET',
    ...
    getSessionHistory: async (
      ...
      try {
        ...
        // Handle potential errors with our helper
        await handleResponseError(response, 'Failed to fetch session history');

        const chatHistory = await response.json();
        console.log(
          `Retrieved ${chatHistory.length} messages for session ${sessionId}`
        );
        return chatHistory;
      } catch (error: unknown) {
        console.error(
    ...
    getSessionDetails: async (
      ...
      try {
        ...
        // Call the backend API to get session details with timeout
        const response = await fetchWithTimeout(
        ...
        // Handle potential errors with our helper
        await handleResponseError(response, 'Failed to get session details');
    ...
    updateSessionTitle: async (
      ...
      try {
        ...
        // Call the backend API to update the session title with timeout
        const response = await fetchWithTimeout(
        ...
        // Handle potential errors with our helper
        await handleResponseError(response, 'Failed to update session title');
