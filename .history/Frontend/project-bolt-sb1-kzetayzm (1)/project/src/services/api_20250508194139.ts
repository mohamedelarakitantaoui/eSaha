...
const API = {
  ...
  auth: {
    ...
    // Find and fix all instances of token parameters that aren't used

    // In resources: section of API (around line 1084)
    resources: {
      /**
       * Get local mental health resources
       */
      getLocalResources: async (
        _token: string, // Changed from "token"
        location: string
      ): Promise<Resource[]> => {
        try {
          console.log(`Getting resources for location: ${location}`);
          // Don't try to use token parameter since we prefixed it with underscore
          ...
          const response = await fetch(
            `/api/resources?location=${encodeURIComponent(location)}`,
            {
              method: 'GET',
              headers: {
                Authorization: `Bearer ${_token}`, // Still use the parameter in headers
              },
            }
          );
          ...
          if (!response.ok) {
            ...
            throw new Error(
              errorData.error || 'Failed to fetch local resources'
            );
          }
      ...
      getResourceDetails: async (
        ...
        try {
          ...
          if (!response.ok) {
            const errorData = (await response.json()) as { error: string };
            throw new Error(
              errorData.error || 'Failed to get resource details'
            );
          }
  ...
  appointments: {
    ...
    bookAppointment: async (
      ...
      try {
        ...
        // If not OK, try to get detailed error information
        if (!response.ok) {
          let errorMessage;
          try {
            const errorData = await response.json();
