...
const ChatPage: React.FC = () => {
  ...
  useEffect(() => {
    // Guard against calling this without user
    if (!user) {
    ...
    const initializeChat = async () => {
      try {
        ...
        // Get token with timeout to avoid infinite waits
        const tokenPromise = getToken();
        const timeoutPromise = new Promise<null>((resolve) => {
          setTimeout(() => {
            console.error('Token retrieval timed out');
            resolve(null);
        ...
        if (isNewSession) {
          ...
          try {
            console.log(
              'About to create session with token:',
            ...
            // FIX: Redirect immediately to the new session to prevent infinite loop
            navigate(`/chat/${newSessionId}`, { replace: true });
            return; // Exit immediately after redirection
          } catch (error) {
