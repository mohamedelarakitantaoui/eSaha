...
import openai
import json
from auth import analyze_sentiment  # Import the sentiment analysis function
    ...
    try:
        ...
        try:
            ...
            mood_entry = {
                ...
                "source": "chat_message",
                "message_id": str(chat_message["_id"]),
                "chat_id": chat_id,
                "created_at": datetime.utcnow(),
                "updated_at": datetime.utcnow()
            }
...
@chat_bp.route('/chat/history/all', methods=['GET'])
@jwt_required()
def get_all_chat_history():
    """Get all chat history for the current user (legacy route)"""
    return chat_history()