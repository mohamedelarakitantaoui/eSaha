...
# -----------------------------
# Chat Endpoint (Protected, for POST)
# -----------------------------
@chat_bp.route('/chat', methods=['POST'])
# For testing purposes, JWT is disabled here.
...
def chat():
    ...
    if not subject_val:
        # Default to "General" if subject is missing
        subject_val = "General"
    ...
    # Validate that subject and message are strings
    ...
    try:
        ...
        # For testing, using a default user_id (e.g., 1) since JWT is disabled.
...
# -----------------------------
# Chat History Endpoint (GET, Protected)
# -----------------------------
@chat_bp.route('/chat-history-test', methods=['GET'])
@jwt_required()
def get_chat_history():
    print("DEBUG: GET /chat-history-test endpoint called")
    # Ensure no JSON body is sent with GET requests!
    # Get the user_id from the JWT token
    user_id = get_jwt_identity()
    print("DEBUG: JWT user_id:", user_id)
    
    # Optional filtering parameter (e.g., filter by subject)
    subject_filter = request.args.get("subject")
    print("DEBUG: subject_filter from query params:", subject_filter)
    ...
    print("DEBUG: Number of chats retrieved:", len(chats))
